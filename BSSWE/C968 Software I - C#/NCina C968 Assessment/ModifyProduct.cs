...
namespace NCina_C968_Assessment
{
    public partial class ModifyProduct : Form
    {
        ...
        {
            //MProduct fields
            idBox.Text = $"{MProduct.ProductID}";
            nameBox.Text = $"{MProduct.Name}";
            priceBox.Text = $"{MProduct.Price}";
            inventoryBox.Text = $"{MProduct.InStock}";
            minBox.Text = $"{MProduct.Min}";
            maxBox.Text = $"{MProduct.Max}";

            candidateList.DataSource = Inventory.Inventory.AllParts;
            associatedPartsList.DataSource = MProduct.AssociatedParts;
        ...
        private void saveButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                MProduct.ProductID = productId;
                MProduct.Name = name;
                MProduct.Price = price;
                MProduct.InStock = inStock;
                MProduct.Min = min;
                MProduct.Max = max;

                Inventory.Inventory.updateProduct(productId, MProduct);
                Close();
            }
            catch (Exception ex)
            {
        ...
        private void addPartButton_Click(object sender, EventArgs e)
        {
            try
            {
                if (candidateList.SelectedRows.Count > 0)
                {
                    ...
                    if (!MProduct.AssociatedParts.Contains(newPart)) //no duplicates
                    {
                        MProduct.addAssociatedPart(newPart);
                        candidateList.ClearSelection();
                    }
                    else
                    {
            ...
            try
            {
                if (associatedPartsList.SelectedRows.Count > 0)
                {
                    ...
                    if (result == DialogResult.Yes)
                    {
                        ...
                        MProduct.removeAssociatedPart(newPart.PartID);
                        associatedPartsList.ClearSelection();
                    }
                    else
                    {
